...
def main(cfg, gpu, save_dir):
    ...
    trainset = eval(dataset_name)(dataset_root, 'train', traintransform, modals)
    valset = eval(dataset_name)(dataset_root, 'val', valtransform, modals)
    class_names = trainset.CLASSES
    num_classes = trainset.n_classes

    # --- 构造模型（统一使用 eval 调用） ---
    model_name = model_cfg['NAME']
    backbone_variant = model_cfg['BACKBONE']
    model = eval(model_name)(backbone_variant, num_classes, modals)
    # --- Auto resume from latest periodic checkpoint if RESUME is empty ---
    if resume_path == '':
        checkpoints = sorted(Path(save_dir).glob("*_checkpoint.pth"), key=lambda p: int(re.findall(r"epoch(\\d+)", str(p))[-1]) if re.findall(r"epoch(\\d+)", str(p)) else -1)
        if checkpoints:
            resume_path = str(checkpoints[-1])
            logger.info(f"[Auto-Resume] Found latest checkpoint: {resume_path}")
    ...
    if os.path.isfile(resume_path):
        ...
        logger.info(f"[Resume] Loaded weights from {resume_path}")
        logger.info(f"[Resume] Resume epoch: {resume_checkpoint['epoch']} | Loss: {resume_checkpoint['loss']:.4f} | Best mIoU: {resume_checkpoint['best_miou']:.2f}")
        logger.info(msg)
    else:
    ...
    # scaler = GradScaler(
    #     enabled         = train_cfg['AMP'],  # 保持原逻辑
    #     init_scale      = 256,               # 默认 2**16 ＝ 65536；先大幅调低
    #     growth_interval = 200,               # 每 200 step 才尝试乘 2
    #     growth_factor   = 2.0,               # 保持默认
    #     backoff_factor  = 0.5                # 保持默认
    # )
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train_loss = 0.0        
        lr = scheduler.get_lr()
        lr = sum([l.real if isinstance(l, complex) else l for l in lr]) / len(lr)
        if lr <= 1e-8:
            lr = 1e-8
        ...
        for iter, (sample, lbl) in pbar:
            ...
            with autocast(enabled=train_cfg['AMP']):
                ...
                if isinstance(outputs, tuple):
                    ...
                    loss = loss_fn(logits, lbl) + loss_aux
                else:
                    logits = outputs
                    loss = loss_fn(logits, lbl)

            scaler.scale(loss).backward()
            scaler.unscale_(optimizer)                          # 解除 loss-scale
            # # 1. 梯度裁剪
            # torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)  # max-norm = 1
            ...
            if (train_cfg['DDP'] and torch.distributed.get_rank() == 0) or (not train_cfg['DDP']):
                ...
                if miou > best_mIoU:
                    ...
                    torch.save({'epoch': best_epoch,
                                ...
                                'best_miou': best_mIoU}, cur_best_ckp)
        ...
        if (epoch + 1) % 50 == 0:
            if (train_cfg['DDP'] and torch.distributed.get_rank() == 0) or (not train_cfg['DDP']):
                ckpt_path = save_dir / f"{model_cfg['NAME']}_{model_cfg['BACKBONE']}_{dataset_cfg['NAME']}_epoch{epoch+1}_checkpoint.pth"
                torch.save({'epoch': epoch + 1,
                            'model_state_dict': model.module.state_dict() if train_cfg['DDP'] else model.state_dict(),
                            'optimizer_state_dict': optimizer.state_dict(),
                            'scheduler_state_dict': scheduler.state_dict(),
                            'loss': train_loss,
                            'best_miou': best_mIoU}, ckpt_path)
                logger.info(f"[Checkpoint] Saved periodic checkpoint at epoch {epoch+1}")
